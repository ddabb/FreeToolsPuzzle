...
function solveMasyu(pearls, size, verbose=false) {
  ...
  function isDeadEnd(){
    for(let d=0;d<N;d++){
      ...
      if(countAvailable(d)===0&&degree[d]<2)return true;
    }
    return false;
  }
  ...
  function propagate(){
    ...
    while(changed){
      ...
      for(let d=0;d<N;d++){
        ...
        // 规则3：度=0且只有2个邻居候选（corner顶点）→ 必须用2条边
        if(degree[d]===0&&adj[d].length===2){
          const nb0=adj[d][0],nb1=adj[d][1];
          if(degree[nb0]<2&&degree[nb1]<2){
            const k0=d<nb0?`${d}-${nb0}`:`${nb0}-${d}`;
            const k1=d<nb1?`${d}-${nb1}`:`${nb1}-${d}`;
            if(!usedEdges.has(k0)&&!usedEdges.has(k1)){
              // 强制连接两条边
              usedEdges.add(k0);usedEdges.add(k1);
              degree[d]=2;degree[nb0]++;degree[nb1]++;
              totalProp++;
              changed=true;
              if(!pearlOK(d)){
                usedEdges.delete(k0);usedEdges.delete(k1);
                degree[d]=0;degree[nb0]--;degree[nb1]--;
                return false;
              }
            }
          }
        ...
        // 规则4：度=1的pearl点，只剩1个可用邻居 → 强制连接
        if(degree[d]===1&&pearlAt[d]){
          const nbUsed=[];
          for(const e of usedEdges){
            const[a,b]=e.split('-').map(Number);
            if(a===d||b===d)nbUsed.push(a===d?b:a);
          }
          if(nbUsed.length===1){
            const usedNb=nbUsed[0];
            // 找唯一一个不在usedEdges中的候选
            let candidates=[];
            for(const nb of adj[d]){
              if(degree[nb]<2&&nb!==usedNb){
                const k=d<nb?`${d}-${nb}`:`${nb}-${d}`;
                if(!usedEdges.has(k))candidates.push(nb);
              }
            }
  ...
  function backtrack(depth){
    ...
    if(allDone){
      if(usedEdges.size===N){
        ...
        if(visited.size===N){
          solutions++;
          if(verbose) console.log('  解 #'+(solutions));
        }
      }
      return;
    }

    if(depth>30)return; // 保险

    const v=chooseVertex();
    if(v===-1)return;

    // 对候选边排序：优先尝试符合pearl约束的方向
    const candidates=[];
    for(const nb of adj[v]){
      if(degree[nb]>=2)continue;
      const k=v<nb?`${v}-${nb}`:`${nb}-${v}`;
      if(usedEdges.has(k))continue;
      // 估算：这条边之后，v的pearl约束是否可能满足
      const type=pearlAt[v];
      candidates.push({nb,k,type});
    }

    // 排序：珍珠点的候选边优先
    if(pearlAt[v]){
      candidates.sort((a,b)=>{
        // 对珍珠点，尝试先选能让pearl约束满足的方向
        const nbUsed=usedEdges.size>0?[]:null;
        // 简化：珍珠点优先
        return pearlAt[v]?0:0;
      });
    }
...
// ---- 测试 ----
const CONFIG={easy:{size:6,pearlDensity:0.35,blackRatio:0.45},medium:{size:8,pearlDensity:0.38,blackRatio:0.5}};
