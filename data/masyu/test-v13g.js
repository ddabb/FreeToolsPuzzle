...
// 测试不同密度
console.log('测试不同密度 (6x6格):');
for(const density of [0.15, 0.20, 0.25, 0.30, 0.35]){
  let solvable=0,unique=0,multi=0,total=0;
  for(let i=0;i<50;i++){
    const dotSize=7,path=generatePath(dotSize);
    if(path.length<dotSize*dotSize*0.5)continue;
    const config={...{size:6,pearlDensity:density,blackRatio:0.45}};
    const cands=genCand(path,dotSize,config);
    if(cands.length<3)continue;
    const pearls=selPearls(cands,dotSize,config);
    if(pearls.length<2)continue;
    total++;
    const result=solveMasyu(pearls,6);
    if(result.solutions>=1)solvable++;
    if(result.isUnique)unique++;
    else if(result.solutions>1)multi++;
    if(total>=20)break;
  }
  console.log(`  密度${(density*100).toFixed(0)}%: ${solvable}/${total}有解, ${unique}唯一解`);
}
