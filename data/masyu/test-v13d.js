  ...
  function backtrack(){
    ...
    if(type&&degree[v]===1){
      ...
      for(const e of usedEdges){
        const[a,b]=e.split('-').map(Number);
        if(a===v)existingNb=b;
        else if(b===v)existingNb=a;
      }
      if(existingNb>=0){
        const er=Math.floor(existingNb/dotSize),ec=existingNb%dotSize;
        const vr=Math.floor(v/dotSize),vc=v%dotSize;
        const edr=er-vr,edc=ec-vc;
        candidates.sort((a,b)=>{
          const ra=Math.floor(a.nb/dotSize),ca=a.nb%dotSize;
          const rb=Math.floor(b.nb/dotSize),cb=b.nb%dotSize;
          const dra=ra-vr,dca=ca-vc,drb=rb-vr,dcb=cb-vc;
          // 珍珠要求：黑珠=转角(点积=0)，白珠=直线(点积!=0)
          // 当前已有方向(edr,edc)，新方向(dr,dc)
          // 黑珠：新方向需垂直 |edr*dr+edc*dc|=0
          // 白珠：新方向需水平 |edr*dr+edc*dc|>0
          const dotA=Math.abs(edr*dra+edc*dca);
          const dotB=Math.abs(edr*drb+edc*dcb);
          if(type==='black')return dotA<dotB?-1:dotA>dotB?1:0;
          else return dotA>dotB?-1:dotA<dotB?1:0;
        });
      }
    }
