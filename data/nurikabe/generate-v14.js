/**
 * Nurikabe Puzzle Generator v14
 * 
 * Same as v11 but with optimized hard parameters.
 * Key insight: hard difficulty just needs bigger rooms and lower black %.
 * The v11 algorithm works perfectly for easy/medium.
 * For hard, we use maxRoom=10, targetBlack=0.40 which gives more slack.
 * 
 * Also adds: merge tiny rooms more aggressively, and post-processing
 * to remove isolated 1-cell rooms (turn them black if possible).
 */
...
const CONFIGS = {
  ...
  hard:   { size: 10, maxRoom: 10, minRoom: 1, targetBlack: 0.40, count: 1000 },
};
...
function generatePuzzle(config) {
  ...
  for (let attempt = 0; attempt < 1000; attempt++) {
    // Step 1: Create rooms using flood-fill
    ...
    // Step 2: Select rooms to turn black
    const grid = Array.from({length: size}, () => Array(size).fill(0));
    
    const blackTargetMin = Math.floor(total * targetBlack);
    const blackTargetMax = Math.floor(total * (targetBlack + 0.12));
    let blackCount = 0;
    
    // Turn small rooms black first
    ...
    // Step 5: Merge rooms that became too small or split rooms that became too large
    // (due to black cell adjustments)
