/**
 * Nurikabe Generator v20 - Optimized v11 for hard with corrected params
 * 
 * Key insight from analyzing existing valid hard puzzles:
 * - Black percentage is 51-66% (not 48%!)
 * - Black cells form "river" patterns (mostly 2 adj)
 * - maxRoom=6 is fine (existing puzzles have rooms up to 6)
 * 
 * Strategy: Use v11 room-first but with higher targetBlack for hard
 * and much higher retry limit
 */
...
const CONFIGS = {
  easy:   { size: 5,  maxRoom: 4, minRoom: 1, targetBlack: 0.44, count: 1000, maxFail: 500 },
  medium: { size: 7,  maxRoom: 5, minRoom: 1, targetBlack: 0.48, count: 1000, maxFail: 500 },
  hard:   { size: 10, maxRoom: 6, minRoom: 1, targetBlack: 0.55, count: 1000, maxFail: 10000 },
};
...
function generatePuzzle(config) {
  ...
  for (let attempt = 0; attempt < 2000; attempt++) {
    ...
    // Step 2: Turn small rooms black (checking 2x2 proactively)
    ...
    // Add more black cells if needed (from borders of existing rooms)
    ...
    // Quick fail: if still way under target, skip
    if (blackCount < blackTargetMin * 0.85) continue;
    
    // Step 3: Fix 2x2 (should be rare with proactive check)
