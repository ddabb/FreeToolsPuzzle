...
function generatePuzzle(config) {
  ...
  for (let attempt = 0; attempt < 1000; attempt++) {
    ...
    const unique = checkUniqueSolution(grid, lines, size);
    if (!unique) continue;
...
function checkUniqueSolution(grid, originalLines, size) {
  ...
  function dfs(r, c, fromDir, visitedCells, path) {
    if (solutionCount > 1) return;
    
    if (path.length > 2 && r === startR && c === startC) {
      const tempLines = pathToLinesFromVisited(path, size);
      if (validateSolution(tempLines, grid, size)) {
        solutionCount++;
      }
      return;
    }
    ...
    const key = `${r},${c}`;
    if (path.length > 0 && visitedCells.has(key)) return;
    
    visitedCells.add(key);
    
    for (let d = 0; d < 4; d++) {
      if (fromDir !== -1 && d === oppositeDir[fromDir]) continue;
      ...
      if (grid[r][c] !== EMPTY) {
        const cell = originalLines[r][c];
        const isStraight = (cell.top && cell.bottom) || (cell.left && cell.right);
        if (fromDir !== -1) {
          const prevDir = fromDir;
          const diff = Math.abs(d - prevDir);
          if (grid[r][c] === BLACK && diff !== 1 && diff !== 3) continue;
          if (grid[r][c] === WHITE && diff !== 2) continue;
        }
      }
      
      path.push([nr, nc]);
      dfs(nr, nc, d, new Set(visitedCells), [...path]);
      path.pop();
  ...
  for (let d = 0; d < 4; d++) {
    ...
    dfs(nr, nc, d, new Set([`${startR},${startC}`]), [[startR, startC], [nr, nc]]);
...
function pathToLinesFromVisited(path, size) {
  const lines = Array.from({ length: size }, () =>
    Array.from({ length: size }, () => ({ top: false, right: false, bottom: false, left: false }))
  );
  ...
  if (r2 === r1 - 1) { lines[r2][c2].bottom = true; lines[r1][c1].top = true; }
  else if (r2 === r1 + 1) { lines[r2][c2].top = true; lines[r1][c1].bottom = true; }
  else if (c2 === c1 - 1) { lines[r2][c2].right = true; lines[r1][c1].left = true; }
  else if (c2 === c1 + 1) { lines[r2][c2].left = true; lines[r1][c1].right = true; }
